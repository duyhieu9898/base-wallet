...
const useCallsData = (
  ...
  const {
    root: { providerStore, multicallStore },
  } = useStores();
  const { activeChainId: chainId } = providerStore.providerStatus;
  const { callResults } = multicallStore;
  ...
  }, [chainId, options, serializedCallKeys, multicallStore]);
...
export function useSingleContractMultipleData(
  ...
  const latestBlockNumber = providerStore.currentBlockNumber;
